...
import { playlistsApi, PlaylistVideo, RecommendedTrack, RemixResult } from '../../../api/youtube';

/**
 * Fetches the three independent pieces of the track detail page in parallel
 * — the track itself, its in-library recommendations, and its YouTube remix
 * search — since none of them depend on each other completing first.
 */
export function useTrackDetail() {
  ...
  return { playlistId: id ?? '', video, recommendations, remixes };
}
