 ...
 * but one of the providers it searches (Soulseek, JioSaavn, or a connected
 * Deezer/Qobuz/Tidal account) turned up real search results that just
 * didn't clear the match-confidence bar (see CloseHqCandidate's own doc
 * comment on the backend, including why the list is capped at 5) — usually
 * because the video's own stored name differs slightly (a remix tag, a
 * diacritic, a feat. credit) from the provider's canonical one. Picking one
 * asks for confirmation, then renames the track to match it.
 */
        ...
        <DialogContent>
          ...
          <List disablePadding>
            {candidates.map((c) => (
              ...
              >
                <ListItemText primary={`${c.artist} - ${c.title}`} />
                <Chip label={PROVIDER_LABEL[c.provider]} size="small" variant="outlined" />
              </ListItemButton>
            ))}
          </List>
        </DialogContent>
