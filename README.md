...
**CI builds an installable APK** on every push to `main`/`master` that touches `mobile/` (or a manual dispatch — see above), alongside the Docker publish jobs: it runs `expo prebuild` to generate the native Android project fresh, then `./gradlew assembleRelease` — a *release* build type (not debug: a debug APK doesn't bundle its JS at all, it loads it from a Metro dev server on whatever machine built it, so it only runs while that machine is reachable — release bundles the JS in at build time instead, so the installed app never depends on anything running anywhere). Signed with our own stable key (see below), not Google's — "release" here describes the build type, not Play Store distribution. It shows up two ways: a `YoutubeVault-apk` workflow artifact (kept 30 days, handy for a specific CI run) and a [GitHub Release](../../releases) tagged `mobile-<short-sha>` with the APK attached (kept indefinitely — this is what the app's own update check reads, see below, and what the web frontend's download prompt links to, see next paragraph).

**The web frontend links straight to the latest GitHub Release's APK asset** (`APK_URL` in `frontend/src/constants.ts`) rather than embedding one in the image — `releases/latest/download/<asset>` is a stable, ever-green URL as long as the uploaded asset's filename never changes (it doesn't: every release uploads it as `YoutubeVault.apk`, see the `android-apk` job). A phone browser opening the web app gets a full-screen prompt (`MobileAppGate`, dismissible) pointing at that URL. This is what decouples the two build pipelines (see above) — the frontend doesn't need anything freshly built from `mobile/` to publish, and `mobile/` changing doesn't need a frontend republish either, since the link resolves to whatever's newest at click time regardless of which frontend image is currently deployed.
